  ...
  const handleSubmit = (e) => {
    ...
    uploadRef.put(img).on(
      ...
      async () => {
        ...
        firestore
          .collection("images")
          .add(imageModel(img.name, url,Title,desc))
          .then(async (doc) => {
          ...
          {progress ? (
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <Form.Group controlId="formBasicSubmit" className="my-2">
                <input type="text" className="border-2 border-black pl-2 w-full mb-3 " placeholder="Title" onChange={(event)=>setTitle(event.target.value)}/>
                <input type="text" className="border-2 border-black pl-2 w-full mb-3 " placeholder="Description" onChange={(event)=>setdesc(event.target.value)}/>
                <Button
                  type="submit"
                  variant="dark"
                  bg="dark"
